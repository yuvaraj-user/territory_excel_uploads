                ...
                <div class="row">
                  <div class="col-md-2 ml-5">
                    <div class="form-check">
                      <input class="form-check-input discount_status" type="radio" name="discount_status" id="flexRadioDefault1" value="budget" checked>
                      <label class="form-check-label" for="flexRadioDefault1">
                        Budget
                      </label>
                    </div>
                    <div class="form-check">
                      <input class="form-check-input discount_status" type="radio" name="discount_status" id="flexRadioDefault2" value="approved">
                      <label class="form-check-label" for="flexRadioDefault2">
                        Approved
                      </label>
                    </div>
                    <div class="form-check">
                      <input class="form-check-input discount_status" type="radio" name="discount_status" id="flexRadioDefault3" value="actual">
                  ...
                  <div class="col-md-6">
                    <label for="formFile" class="form-label" style="font-weight:bold;">Upload discount excel</label>
                    <input class="form-control" type="file" name="budget_excel" id="budget_excel_file">
                  </div>
                  <div class="col-md-3 m-auto">
                    <button type="submit" class="btn btn-primary" id="file_upload"><i class="fas fa-upload"></i> Upload</button>
                    <a href="assets/samples/discount_budget_sample.xlsx" id="sample_link"><button type="button" class="btn btn-info"><i class="fas fa-download"></i> <span id="sample_text">Budget sample file</span></button>
                  </div>
                </div>
              </div>
            </div>
...
<script type="text/javascript">
  ...
  $(document).on('click','#file_upload',function(e){
    e.preventDefault();
    var file_data        = $('#budget_excel_file').prop('files')[0];
    var discount_type    = $('.discount_status:checked').val();
    if(file_data) {
      var form_data        = new FormData();
      form_data.append("Action","discount_budget_save");
  ...
  $(document).on('change','.discount_status',function(){
    ...
    if(value == 'budget') {
      ...
      $('#sample_link').attr('href',"assets/samples/discount_budget_sample.xlsx");
      $('#sample_text').text('Budget sample file');
    } else if(value == 'approved') {
      $('#sample_link').removeAttr('href');
      $('#sample_link').attr('href',"assets/samples/discount_approved_sample.xlsx");
      $('#sample_text').text('Approved sample file');
    } else if(value == 'actual') {
      $('#sample_link').removeAttr('href');
      $('#sample_link').attr('href',"assets/samples/discount_actual_sample.xlsx");
      $('#sample_text').text('Actual sample file');
    }
  });
